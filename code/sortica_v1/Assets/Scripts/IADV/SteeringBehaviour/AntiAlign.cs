...
public class AntiAlign : SteeringBehaviour
{
    ...
    /*
     * Para el Steering Behaviour AntiAlign, el objetivo es que el Agente agent mire en, aproximadamente, la
     * dirección opuesta a la del Agente target(es decir, que su orientación esté dentro del ángulo interior de visión opuesto
     * al target). Para ello, se realizan los siguientes pasos:
     * 
     *      (1) Se calcula la orientación relativa del target con respecto al ángulo opuesto al agente. Es decir, si la orientación
     *      del agente fuera el punto origen de la rotación (el 0), qué ángulo tendría el target. Por ejemplo, si
     *      agent.Orientation = 45º y target.Orientation = 30º, la orientación relativa al ángulo objetivo sería 
     *      45º - (30º+180º) = -165º que, en el rango [0, 360], es 360º-165º = 195º.
     *      
     *    # A partir de aquí se procede como en Align:
     *      (2) Se calculan
     *      
     *          (2.1) La diferencia real de orientación (diferenciaRealOrientacion), que es la distancia más pequeña entre
     *          las orientaciones: min{|agent.Orientacion - targetOrientation|, 360 - |agent.Orientacion - targetOrientation|}.
     *          
    ...
    {
        float targetOrientation = Bodi.MapToRange(target.Orientacion - 180, 0, 360);

        // (1)
        float targetOrientationRelativa = Bodi.MapToRange(agent.Orientacion - targetOrientation, 0, 360);
